...
<main class="container">
  ...
  <form class="pure-form" action="<c:url value='/users'/>" method="post" style="display:grid; gap:12px; max-width:520px">
    ...
    <button type="submit" class="btn" style="width:max-content">Đăng ký</button>
  </form>

  <!-- Hiển thị các email đã đăng ký trong phiên (session 'subs') -->
  <c:if test="${not empty sessionScope.subs}">
    <section style="margin-top:24px">
      <h3>Email đã đăng ký trong phiên</h3>
      <ul class="list">
  ...
  <p style="margin-top:20px">
    <a href="<c:url value='/users'><c:param name='view' value='list'/></c:url>">← Về trang chủ</a>
  </p>
</main>
